...
export default function App() {
  ...
  // Poll Java server API
    // Realtime подписка на Firebase
    useEffect(() => {
        // Слушаем ветку 'live_results', в которую отправляет Java
        ...
        const unsubscribe = onValue(
            ...
            (snapshot) => {
                ...
                if (data) {
                    // Если Java отправляет массив групп напрямую
                    if (Array.isArray(data)) {
                        setApiResults(data);
                    } else {
                        // Если данные приходят как объект хэшей
  ...
  // Identify latest finished athlete
  const latestFinisher = useMemo(() => {
    let participant: Participant | null = null;
    let groupName = "";

    for (const group of apiResults) {
      const latestInGroup = [...group.finishers, ...group.dnfs, ...group.dsqs, ...group.dnss].find(p => p.isLatest || (p as any).latest);
      if (latestInGroup) {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans">
      ...
      <header className="bg-white/95 backdrop-blur-xs border-b border-slate-200/80 fixed top-0 left-0 right-0 z-50 shadow-xs px-2 md:px-4 py-2">
        <div className="max-w-7xl mx-auto flex flex-row items-center justify-between gap-1.5 md:grid md:grid-cols-3 md:gap-4">
          ...
          {/* Right: Clock with status color */}
          <div className="flex items-center justify-end">
            <div className={`flex items-center gap-1 md:gap-2 border rounded-md md:rounded-xl px-1.5 py-0.5 md:px-3 md:py-1.5 text-[10px] md:text-xs font-mono transition-colors ${
              apiError
                      ...
                      {latestFinisher.participant.status === "FINISHED" ? (
                        ...
                        <span
                          className={`text-lg md:text-2xl font-black font-mono leading-none ${
                            latestFinisher.participant.status === "DNF"
                              ? "text-red-500"
                    ...
                    >
                      ...
                      <div className="bg-slate-50/50 border-b border-slate-100 px-3 py-2 md:px-6 md:py-4 flex justify-between items-center gap-2">
                        ...
                        <span className="text-[9px] md:text-[10px] bg-slate-200/60 text-slate-600 font-black px-1.5 py-0.5 rounded-md uppercase tracking-wider shrink-0">
                          🏁 {g.finishers.length + g.dnfs.length + g.dsqs.length + g.dnss.length} уч.
                        </span>
                      </div>

                      {/* Group Body */}
                      <div className="p-2 md:p-4 flex-1 flex flex-col gap-1 md:gap-1.5">
                        {g.finishers.length === 0 &&
                        g.dnfs.length === 0 &&
                        g.dsqs.length === 0 &&
                        g.dnss.length === 0 ? (
                            ...
                            {g.finishers.map(f => {
                              ...
                              return (
                                <div
                                  ...
                                  className={`flex items-center justify-between p-2 md:p-3 rounded-xl md:rounded-2xl border transition-all cursor-pointer hover:border-blue-300 hover:shadow-sm active:scale-[0.99] group/row ${
                                    isLatest
                                ...
                                >
                                  <div className="flex items-center gap-2 md:gap-3 min-w-0">
                                    <div
                                      className={`w-5.5 h-5.5 md:w-6 md:h-6 rounded-md md:rounded-lg flex items-center justify-center font-bold text-[10px] md:text-xs shrink-0 ${
                                        f.rank === "1"
                                    ...
                                    <span
                                      className={`text-[11px] md:text-xs uppercase truncate group-hover/row:text-blue-600 transition-colors ${
                                        isLatest ? "font-bold text-blue-950" : "font-semibold text-slate-700"
                                      }`}
                                    >
                                      {f.name}
                                    </span>
                                  ...
                                  <div className="flex items-center gap-1.5 md:gap-2 shrink-0">
                                    <span
                                      className={`text-[10px] md:text-xs font-mono font-bold px-1.5 py-0.5 rounded-md md:rounded-lg transition-colors group-hover/row:bg-blue-100 group-hover/row:text-blue-700 ${
                                        isLatest
                            ...
                            {[...g.dnfs, ...g.dsqs, ...g.dnss].map(e => {
                              ...
                              return (
                                <div
                                  ...
                                  className={`flex items-center justify-between p-2 md:p-3 rounded-xl md:rounded-2xl border transition-all cursor-pointer hover:border-slate-300 hover:shadow-sm active:scale-[0.99] group/row ${
                                    isLatest
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-1.5 md:gap-2 shrink-0">
                                    <span
                                      className={`text-[10px] md:text-xs font-mono font-black px-1.5 py-0.5 rounded-md md:rounded-lg border transition-colors group-hover/row:bg-slate-100 ${
                                        e.status === "DNF"
      ...
      <AnimatePresence>
        {selectedAthlete && (
          ...
          >
            ...
            >
              {/* Top Colored Banner based on rank or status */}
